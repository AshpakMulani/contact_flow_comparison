...
function cleanObject(obj){
  ...
  //var conf = { "IGNORE_TAG" : ["entryPointPosition","position","isFriendlyName","conditionMetadata"] }
...
function flowCompare(oldFlow, newFlow){ 
  ...
  /*
  updatedBlocks structure. This is list of object containing details for each
  block with its original metadata and action details along with diff for them

  [
    {
      Id = <blockId>
      metadata = <metadata from contact flow>
      metadataDiff = <diff details for metadata>
      action = <action from contact flow>
      actionDiff = <diff details for action>
    }
  ]
  ...
  */
  ...
  console.log(uiRemovedBlocks)
  console.log(uiUpdatedBlocks)


  //drawDiff(uiUpdatedBlocks)
